# coding=utf-8
"""
社区检测算法 - Python 3.12 版本
功能：遍历 alpha 值 (0-1.0)，输出 11 个版本结果并绘制模块度折线图
"""
import numpy as np
...
class CDDAWN_CDSSA_Fix:
    ...
    def preprocess(self, G, node_attributes):
        ...
        if node_attributes and self.precomputed_S_A is None:
            ...
            print("⚠️ 无节点属性且未提供 precomputed_S_A，后续属性相似度将为 0")
    ...
    def simplify_network(self):
        ...
        # 根据公式 E′ = {⟨vi, vp⟩|MD(vi, vp) = maxvjVMD(vi, vj)}重新构建网络
        # 为每个节点 vi 找到 MD(vi, vj) 最大的 vj
        selected_edges = {}
        for u in self.nodes:
            # 找到节点 u 与其他节点的 MD 最大值
            max_md_value = -1
            max_md_target = None
            for v in self.nodes:
                if u == v:
    ...
    def detect_communities(self):
        ...
        print("全局 A_avg 中位数：{0:.4f}, IS 中位数：{1:.4f}".format(global_A_avg, IS_avg))
        ...
        iteration = 0
        while U and len(C) < 30:  # 限制最大社区数为 30 个，避免碎片化
            iteration += 1
            if iteration % 3 == 0:
            ...
            if len(T_r) >= self.min_split_size:
                ...
                # 分裂条件：MD < 全局 A_avg 中位数且 IS > IS 中位数
        ...
        # 处理剩余未分裂节点
        print("正在分配剩余未分裂节点...")
        for node in U.copy():  # 使用 copy 避免在迭代中修改集合
            best_comm = None
            best_sim = -1
    ...
    def save_results(self, communities, node_communities, id_to_city=None, year=None):
        ...
        # 输出目录
        output_dir = r"C:\Users\hp\Desktop\alpha社区结果"
        ...
        with codecs.open(filename, "w", encoding="utf-8") as f:
            f.write("CDSSA 优化版算法结果（仅保留 4 个核心修改）\n")
            f.write("=" * 50 + "\n")
    ...
    def run(self, G, node_attributes=None, id_to_city=None, precomputed_S_A=None, year=None):
        print("=" * 60)
        print("开始运行仅保留 4 个核心修改的 CDSSA 算法")
        ...
        modularity = self.evaluate_communities(communities)
        self.visualize_results(communities, year=year)
        return communities, node_communities, modularity
...
def plot_modularity_vs_alpha(alpha_values, modularity_values, year=None):
    """绘制模块度 Q 与 alpha 值的折线图"""
    try:
        import matplotlib.pyplot as plt
        plt.rcParams['font.sans-serif'] = ['SimHei']
        plt.rcParams['axes.unicode_minus'] = False
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(alpha_values, modularity_values, marker='o', linestyle='-', 
        ...
        # 设置标签和标题
        plt.xlabel('α值 (结构相似度权重)', fontsize=12, fontweight='bold')
        plt.ylabel('模块度 Q', fontsize=12, fontweight='bold')
        plt.title('{0}年 - 模块度 Q 随α值变化曲线'.format(year if year else ''), 
        ...
        # 在每个点标注具体数值
        for i, (alpha, q) in enumerate(zip(alpha_values, modularity_values)):
            plt.annotate('{0:.4f}'.format(q), 
                        xy=(alpha, q), 
                        xytext=(0, 5),
        ...
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        prefix = "{}_".format(year) if year else ""
        png_filename = os.path.join(output_dir, "{}modularity_vs_alpha.png".format(prefix))
        plt.savefig(png_filename, dpi=300, bbox_inches='tight')
        plt.close()
        print("模块度-Q 折线图已保存至：{}".format(png_filename))
...
def main():
    ...
    # 遍历 alpha 值：0, 0.1, 0.2, ..., 1.0
    alpha_values = [round(i * 0.1, 1) for i in range(11)]
    modularity_results = []
    
    print("\n" + "=" * 60)
    print("开始遍历 alpha 值：{0}".format(alpha_values))
    print("=" * 60)
    
    for alpha in alpha_values:
        print("\n" + "=" * 60)
        print("当前运行 alpha = {0}".format(alpha))
        print("=" * 60)
        ...
        # 运行（传入 year 和 alpha 标识）
        year_alpha = "{0}_alpha{1}".format(year, str(alpha).replace('.', '_'))
        communities, node_communities, modularity = detector.run(
            G=G,
            node_attributes={},
        ...
        print("\nAlpha={0:.1f} 运行完成，模块度 Q={1:.4f}".format(alpha, modularity))

    # 绘制模块度-Q 折线图
    print("\n" + "=" * 60)
    print("正在绘制模块度-Q 折线图...")
    print("=" * 60)
    plot_modularity_vs_alpha(alpha_values, modularity_results, year=year)
    
    # 输出汇总结果
    print("\n" + "=" * 60)
    print("运行完成！")
    print("=" * 60)
    print("输出文件已保存至：C:\\Users\\hp\\Desktop\\alpha社区结果")
    print("\nAlpha 遍历结果汇总：")
    print("-" * 60)
    print("{0:<10} {1:<10}".format("Alpha", "模块度 Q"))
    print("-" * 60)
    for alpha, q in zip(alpha_values, modularity_results):
        print("{0:<10.1f} {1:<10.4f}".format(alpha, q))
    print("-" * 60)
    
    # 找出最优 alpha
    best_idx = np.argmax(modularity_results)
    print("\n最优 Alpha 值：{0:.1f} (模块度 Q={1:.4f})".format(
        alpha_values[best_idx], modularity_results[best_idx]))
